...
export async function seed() {
  ...
  // ── Customers from CSV ───────────────────────────────────────────────────
  console.log("Importing customers from Contacts.csv...");

  const csvContent = readFileSync("./Contacts.csv", "utf8");
  ...
  console.log(`Processing ${customerValues.length} customers from CSV (skipped ${skippedCount})`);
  ...
  console.log(
    `Seeded: 3 payment methods, 1 demo user (${DEMO_EMAIL} / ${DEMO_PASSWORD}), ` +
      `${customerValues.length} customers (from CSV), ${productValues.length} products, ` +
