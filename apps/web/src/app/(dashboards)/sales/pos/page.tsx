	...
	useEffect(() => {
		if (resumeOrder && resumeOrder.orderItems && cart.length === 0) {
			const restoredCart = resumeOrder.orderItems.map((item: any) => {
				...
				const name = prod.name || `Item #${item.product_id}`;
				
				// Weight detection
				let group: "weight" | "volume" | "none" = "none";
				let activeUnit: "KG" | "GM" | "L" | "ML" | "UNIT" = "UNIT";
				const unit = (prod.unit || "").toLowerCase();
				const lowerName = name.toLowerCase();
				if (unit.includes("kg") || unit.includes("kilogram") || lowerName.includes(" kg") || lowerName.includes("kg ") || lowerName.endsWith("kg")) {
					group = "weight";
					activeUnit = "KG";
				} else if (unit.includes("gm") || unit.includes("gram") || lowerName.includes(" gm") || lowerName.includes("gm ") || lowerName.endsWith("gm") || lowerName.includes("gram")) {
					group = "weight";
					activeUnit = "KG";
				} else if (unit.includes("ml") || lowerName.includes("ml") || lowerName.includes("ml ") || lowerName.endsWith("ml")) {
					group = "volume";
					activeUnit = "L";
				} else if (unit === "l" || unit.includes("liter") || unit.includes("litre") || lowerName.includes(" l ") || lowerName.endsWith(" l") || lowerName.includes("liter") || lowerName.includes("litre")) {
					group = "volume";
					activeUnit = "L";
				}
