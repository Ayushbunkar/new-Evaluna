...
| `trade_name` | text | Trade name (optional) |
| `tax_id` | text | Tax ID (14 digits) |
| `state_tax_id` | text | State tax ID (IE) |
| `tax_regime` | text | CRT: "1" (Simples), "2" (excess), "3" (Normal) |
| **Address** | | |
| `state_code` | text | State code (e.g. "SP") |
| `city_code` | text | IBGE city code |
| `city_name` | text | City name |
| `street` | text | Street |
| `street_number` | text | Number |
| `district` | text | District |
| `zip_code` | text | Postal code (8 digits) |
| `address_complement` | text | Additional address details (optional) |
| **SEFAZ** | | |
...
| `operation_nature` | text | Operation nature (e.g. "Sale") |
