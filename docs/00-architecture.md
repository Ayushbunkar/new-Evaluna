...
The project uses a Turborepo monorepo. The pure fiscal logic lives in a standalone package (`@evaluna/fiscal`) at `packages/fiscal/`, with zero database dependencies. The DB-coupled files (repositories and invoice service) live in the app layer at `apps/web/src/lib/`.
...
All files in the **`@evaluna/fiscal`** package live under `packages/fiscal/src/`. The three DB-coupled files live in `apps/web/src/lib/`.
